...
bool ModifiedDEtaInSeed::extrapolate(const reco::GsfElectron& aEle, const reco::TrackBase& addTrk,
                                     const math::XYZPoint& beamSpotPos,
                                     const MagneticField& magField,
                                     const GeometricSearchTracker& geoSearchTracker,
  ...
  // auto mtsTransform = std::make_unique<MultiTrajectoryStateTransform>(trackerHandle.product(),&magField);
  ...
  const auto& pixelBarrelLayers = geoSearchTracker.pixelBarrelLayers();
  ...
  auto gsfPropagator = std::make_unique<GsfPropagatorAdapter>(AnalyticalPropagator(&magField));
...
ModifiedDEtaInSeed::variables ModifiedDEtaInSeed::value(const reco::GsfElectron& aEle,
                                                        ...
                                                        const MagneticField& magField,
                                                        const GeometricSearchTracker& geoSearchTracker,
                                                        const CaloGeometry& caloGeomRef,
                                                        const CaloTopology& caloTopoRef) {
  ...
  if ( extrapolate(aEle,addTrk,beamSpot.position(),magField,geoSearchTracker,scAtVtx,seedAtCalo) ) {
  ...
  const float phi1stGSF = reco::reducePhiRange( -( aEle.deltaPhiSuperClusterTrackAtVtx() - aEle.superCluster()->phi() ) );
  const float eta2ndGSF = -( dEtaInSeed2nd - aEle.superCluster()->seed()->eta() );
  const float phi2ndGSF = reco::reducePhiRange( -( dPhiInSC2nd - aEle.superCluster()->phi() ) );
  ...
  const CaloGeometry* caloGeom = &caloGeomRef;
  const CaloTopology* caloTopo = &caloTopoRef;
