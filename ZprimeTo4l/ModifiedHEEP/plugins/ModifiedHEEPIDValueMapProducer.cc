...
void ModifiedHEEPIDValueMapProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  const auto& geoSearchTracker = iSetup.getData(geoSearchToken_);
  ...
  for (const auto& ele : *eleHandle) {
    const auto& additionalGsfTrk = trkIsoCalc_.additionalGsfTrkSelector(ele,gsfTrkHandle,ttBuilder);
    auto additionalCand = trkIsoCalc_.additionalPackedCandSelector(ele,candHandles,candVetos,ttBuilder);
