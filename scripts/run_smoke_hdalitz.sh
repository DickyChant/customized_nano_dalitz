...
MON=$!
cmsRun smoke_hdalitz_cfg.py > cmsrun_hdalitz.log 2>&1; RC=$?
kill $MON 2>/dev/null
echo "### cmsRun RC=$RC"
grep -iE 'Fatal|Exception|Error|feature size|xgboost' cmsrun_hdalitz.log | head -8
echo "### validate"
python3 - <<'PY'
import ROOT
ROOT.gErrorIgnoreLevel = ROOT.kError
f=ROOT.TFile.Open("nanoHDalitz.root"); assert f and not f.IsZombie(),"no output"
